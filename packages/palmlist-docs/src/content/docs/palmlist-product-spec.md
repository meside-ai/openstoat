---
title: Palmlist Product Specification (v3)
---

# Palmlist Product Specification (v3)
...
**One-line description**: Palmlist is a local-first, CLI-first task orchestration system where AI Agents are the default execution engine and humans join only when needed.

**Product characteristics**:
- Open-source project
- Team model target: 1+ Humans and N Agents
- Architecture: Local-first + CLI-first
- No built-in LLM calls inside Palmlist
...
5. **Palmlist stores and orchestrates**: external agents provide intelligence and execution logic.
6. **CLI is the agent super-manual**: CLI help and examples must be written in an agent-skill style that tells agents exactly how to operate Palmlist step by step.

---

## 3. Roles and Permissions

Palmlist defines four roles:
...
Default insertion authority belongs to planners: `Agent Planner` and `Human Planner` can insert tasks into the Palmlist Kanban.
...
> Palmlist does not run LLM inference. It manages storage, state transitions, and coordination APIs/CLI.
...
1. create a `human_worker` task (via `palmlist task create` with `owner=human_worker`)
...
- rollback must be executed through dedicated CLI command `palmlist task self-unblock`
...
- explicit step commands are required for status transitions; there is no generic `palmlist task update --status ...` command
- `palmlist task self-unblock`: dedicated rollback command for `in_progress -> ready`; must include at least one new `--depends-on`
...
# palmlist task create ... --depends-on task_001 --depends-on task_002 ...
...
palmlist task start task_001 --as agent_worker --logs-append "Started implementation work"

# if AI gets stuck: create human task, set dependency, set own task to ready
palmlist task create --project project_checkout_rebuild --title "Provide Paddle API key" \
...
palmlist task self-unblock task_001 \
  --depends-on task_002 \
