---
title: Palmlist Use Cases (v3)
---

# Palmlist Use Cases (v3)
...
- **Storage**: `~/.palmlist/` (SQLite)
- **No account/API Key** required
- **Project is mandatory**: every task belongs to one project; project and template are bound
- **No built-in LLM**: Palmlist orchestrates; external agents provide intelligence
- **CLI is the agent manual**: `palmlist --help` is the operational guide for both agents and humans
...
2) Agent Planner creates Task A (owner=agent_worker, status=ready)
   palmlist task create --project project_checkout_rebuild \
...
3) Agent Worker claims and executes Task A
   ...
   palmlist task done task_001 --output "Implemented and tested" \
...
4) Agent Worker runs self-unblock (dedicated command, not generic status update)
   palmlist task self-unblock task_X --depends-on task_H \
...
- Rollback is allowed only via `palmlist task self-unblock` with at least one new `--depends-on`
...
1) Initialize project with template
   palmlist project init --id project_checkout_rebuild \
