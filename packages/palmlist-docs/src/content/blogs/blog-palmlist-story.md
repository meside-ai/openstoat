---
title: 从想法到产品：Palmlist 的诞生记
---

# 从想法到产品：Palmlist 的诞生记
...
当时的背景是：有一个叫 Palmlist（当时还叫 TaskQueue）的工具，定位是 AI Agent 之间的消息队列。但实际使用中发现一些问题——最核心的问题是：**记忆系统被切断了**。
...
一开始我假设 Palmlist 需要内置 LLM 来做任务规划。但 chase 提出了更简洁的方案：

> "Palmlist 更像一个记忆系统 + 监听器。它不配置 LLM，是外部 Agent（如 OpenClaw、Claude Code）根据 Palmlist 的 CLI 读取模板、规划任务、写入系统。"
...
```bash
$ palmlist need-human task_001 --reason "发现 xxx，需要确认"
```
...
3. **解耦思维**: Palmlist 只管存储，Agent 负责智能
...
Palmlist 将是一个开源项目，目标用户是那些 "1 个人类 + 10 个 AI Agents" 的现代开发团队。

---

*本文记录于 2026-02-25，Slack #ask-palmlist 频道的讨论内容整理。*

*如果你对 Palmlist 感兴趣，欢迎关注项目动态。*