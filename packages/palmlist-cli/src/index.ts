#!/usr/bin/env bun
/**
 * Palmlist CLI - Agent super-manual for task orchestration
...
const cli = yargs(hideBin(process.argv))
  ...
  .usage(
    'Usage: $0 <command> [options]\n\n' +
      'Palmlist orchestrates tasks between AI agents and humans. Local-first, CLI-first.\n' +
      'Storage: ~/.palmlist/ (SQLite). No account/API Key required.\n\n' +
      'WHEN TO USE: Planning work, creating tasks, claiming/executing tasks, handing off to humans,\n' +
      'self-unblocking when blocked. Run \'palmlist manual\' for full agent operational manual.\n\n' +
      'Commands:\n' +
      ...
      '  daemon init       Interactively create .palmlist.json\n' +
      ...
      '  web                Start Web UI server (http://localhost:18008)\n' +
      '  manual             Print full agent operational manual (SKILL-style)'
  )
  .command({
    command: 'web',
    describe: 'Start Web UI server. Opens http://localhost:18008 (set PORT env to change).',
    handler: async () => {
      await import('palmlist-web');
    },
  })
  .command({
    command: 'manual',
  ...
  .demandCommand(1, 'Specify a command. Run palmlist --help or palmlist manual for details.')
  .strict()
  .help()
  .alias('h', 'help')
  .version('0.1.0')
