...
1. **Before creating tasks** (mandatory): List existing tasks to avoid duplicates.
   \`palmlist task ls --project <project_id> --status ready,in_progress\`
...
1. **Claim**: \`palmlist task claim <task_id> --as agent_worker --logs-append "..."\`
2. **Start**: \`palmlist task start <task_id> --as agent_worker --logs-append "..."\`
3. **Done**: \`palmlist task done <task_id> --output "..." --handoff-summary "..." --logs-append "..."\`
   - handoff-summary: min 200 chars. Describe what was done and context for downstream.

### Agent Worker (Blocked by Human)

1. Create human task: \`palmlist task create --project X --owner human_worker --task-type credentials ...\`
2. Self-unblock: \`palmlist task self-unblock <my_task_id> --depends-on <human_task_id> --logs-append "Blocked: ..."\`
...
### daemon init
Interactively create \`.palmlist.json\` (project, agent) in current directory.
...
Start worker daemon. Polls for ready agent_worker tasks and invokes external agents.
  ...
  Config: read from \`.palmlist.json\` in current directory. Example: \`{ "project": "<project_id>", "agent": "/path/to/agent" }\`
...
palmlist task ls --project proj_1 --status ready,in_progress
palmlist task create --project proj_1 --title "Add Paddle mapping" --description "..." \\
...
palmlist task claim task_001 --as agent_worker --logs-append "Claimed"
palmlist task start task_001 --as agent_worker --logs-append "Started"
palmlist task done task_001 --output "Implemented" \\
...
palmlist task create --project proj_1 --title "Provide API key" --description "..." \\
  --acceptance-criteria "Key delivered" --status ready --owner human_worker --task-type credentials
palmlist task self-unblock task_X --depends-on task_H --logs-append "Blocked: need API key"
`;
