---
name: palmlist-planner
description: Plans and creates tasks in Palmlist via CLI. Decomposes work into tasks, inserts tasks into Palmlist Kanban, and manages task dependencies. Use when acting as Agent Planner, when the user mentions plan tasks, break down work, create tasks, palmlist, kanban, task planning, or work decomposition.
metadata: {"openclaw":{"requires":{"bins":["palmlist"]},"emoji":"📋"}}
---

# Palmlist Planner

Agent Planner inserts tasks into Palmlist Kanban. Use the CLI as the operational manual.
...
Project ID comes from `.palmlist.json` in the current directory:
...
Use this `project` value as `--project` in all task commands. Ensure the project exists: `palmlist project ls`.
...
| Action | Command |
|--------|---------|
| List projects | `palmlist project ls` |
| List unfinished tasks | `palmlist task ls --project <project> --status ready,in_progress` |
| Create task | `palmlist task create --project <project> --title "..." --description "..." --acceptance-criteria "..." --status ready --owner agent_worker --task-type implementation` |
| Create with deps | Add `--depends-on task_001 --depends-on task_002` |

**Note**: `<project>` is the `project` value from `.palmlist.json`. Read it before running commands.
