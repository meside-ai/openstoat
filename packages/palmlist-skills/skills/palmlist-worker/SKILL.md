---
name: palmlist-worker
description: Claims and executes tasks in Palmlist via CLI. Runs palmlist task claim, start, done, and self-unblock. Use when acting as Agent Worker, when the user mentions execute task, claim task, complete task, handoff, self-unblock, blocked, palmlist worker, or task execution.
metadata: {"openclaw":{"requires":{"bins":["palmlist"]},"emoji":"🔧"}}
---

# Palmlist Worker

Agent Worker claims and executes tasks from Palmlist Kanban. Use the CLI as the operational manual.
...
palmlist task claim <task_id> --as agent_worker --logs-append "Claimed, starting work"

# Step 2: Start (record progress)
palmlist task start <task_id> --as agent_worker --logs-append "Started implementation"
...
Project ID comes from `.palmlist.json` in the current directory. Use it as `--project` when creating human tasks (e.g. in self-unblock flow).
...
```bash
# 1. Create human task first (use project from .palmlist.json or PALMLIST_PROJECT_ID)
palmlist task create --project <project> \
...
palmlist task self-unblock <task_id> --depends-on <human_task_id> \
...
When `palmlist daemon start` invokes you, the daemon sets:

- `PALMLIST_TASK_ID`: The task ID to work on (use this as `<task_id>` in all commands)
- `PALMLIST_PROJECT_ID`: The project ID (use as `--project` when creating human tasks)

Use `PALMLIST_TASK_ID` as the task to claim, start, and complete. Do not ask the user for a task ID when this env is set.
...
- **Do not** use generic status update — there is no `palmlist task update --status ...`; use explicit `claim`, `start`, `done`, `self-unblock`
...
| Action | Command |
|--------|---------|
| Claim task | `palmlist task claim <id> --as agent_worker --logs-append "..."` |
| Start task | `palmlist task start <id> --as agent_worker --logs-append "..."` |
| Complete task | `palmlist task done <id> --output "..." --handoff-summary "..." --logs-append "..."` |
| Self-unblock | `palmlist task self-unblock <id> --depends-on <human_task_id> --logs-append "..."` |
| Show task | `palmlist task show <id>` |

**Note**: `<project>` is the `project` value from `.palmlist.json`. Use it when creating human tasks in self-unblock flow.
