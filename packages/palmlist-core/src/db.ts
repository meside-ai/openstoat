 ...
 * Storage location: ~/.palmlist/palmlist.db
 */
...
const PALMLIST_DIR = join(process.env.HOME || process.env.USERPROFILE || '.', '.palmlist');
const DEFAULT_DB_PATH = join(PALMLIST_DIR, 'palmlist.db');

let dbInstance: Database | null = null;
let dbPathOverride: string | null = null;

function ensureDir(): void {
  if (!existsSync(PALMLIST_DIR)) {
    mkdirSync(PALMLIST_DIR, { recursive: true });
  }
}

function getDbPathInternal(): string {
  if (dbPathOverride !== null) return dbPathOverride;
  return process.env.PALMLIST_DB_PATH || DEFAULT_DB_PATH;
}
