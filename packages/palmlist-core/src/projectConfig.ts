/**
 * Project-local config from .palmlist.json in current directory.
...
export interface PalmlistConfig {
...
const CONFIG_FILENAME = '.palmlist.json';

/**
 * Load .palmlist.json from the given directory (default: process.cwd()).
 * Returns null if file does not exist or is invalid.
 */
export function loadProjectConfig(cwd?: string): PalmlistConfig | null {
...
/**
 * Write .palmlist.json to the given directory (default: process.cwd()).
 */
export function saveProjectConfig(config: PalmlistConfig, cwd?: string): void {
  const dir = cwd ?? process.cwd();
  const path = join(dir, CONFIG_FILENAME);
